...
一键格式化毕业论文。双击 exe，选文件，点"开始格式化"，搞定。**最推荐把word复制粘贴在txt文件，txt文件自动排版效果最好。**
...
- GUI 界面，所有参数可视化调整
- 页面设置（页边距、装订线、页眉页脚距离）
- 字体字号（正文、各级标题、图表题、脚注，按级别独立配置）
- 自动生成封面和声明页（也支持上传自定义封面）
- 特殊标题格式化（摘要、目录、参考文献、致谢，显示文字可配置）
- 标题自动编号修正（支持 "第X章" 和纯数字两种编号格式）
- 题序与标题间距规范化（统一为 1 字距）
- 图表题注格式化 + 编号连续性检查
- 参考文献悬挂缩进
- 三线表格式
- 目录生成与分级字体修正（一级条目 / 其余条目独立配置，需 Word COM）
- 页码设置（前置罗马数字 + 正文阿拉伯数字，支持奇偶页不同位置）
- 页眉设置（支持奇偶页不同内容、首页不显示、底部边框线）
- 引用逗号间距自动修正
- 配置保存/加载，方便分享给同校同学
...
直接输入 Word 文档时，工具会自动识别标题：

- 自动检测未应用标题样式的段落（"第X章"、"X.X"、"X.X.X" 等编号格式）
- 仅匹配独立标题行（≤50 字符且不以句号结尾），不会误识别段落首句
- 自动识别后保留原文编号，不做重编号

> **注意事项**
> - 编号与标题文字之间需要有空格（如 `2.1 数字劳动`），无空格写法（`2.1数字劳动`）暂不支持
> - 标题必须顶格书写，行首不能有空格，否则无法识别
